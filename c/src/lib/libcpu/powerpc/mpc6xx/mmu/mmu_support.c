/* mmu_support.c
 *
 * MMU support for MPC6xx MMU initialization and exception handling.
 */
 ...
 * Copyright (c) 2010 Aanjhan Ranganathan.
 *
...
static int search_empty_pte_slot(mpc6xx_mm_mme *pteg){
  int i;
  for(i = 0; i < 8; i++) {
...
static int search_valid_pte(mpc6xx_mm_mme *pteg, uint32_t vsid, uint32_t api)
{
...
void get_pteg_addr(mpc6xx_mm_mme** pteg, uint32_t hash)
{
...
/* This function shall be called upon exception on the DSISR
   ...
   - Anything else? */
static int mmu_handle_dsi_exception(BSP_Exception_frame *f, unsigned vector)
{
  uint32_t ea, sr_data, vsid, pi, hash1, hash2, key, api, alut_access_attrb;
  int ppteg_search_status, spteg_search_status;
  int status, pp, wimg;
  mpc6xx_mm_mme* ppteg;
  mpc6xx_mm_mme* spteg;
  unsigned long cause, msr;
  ...
  if ( ea > (int)RamSize ) {
    /* FIXME: Use fatal error handling */
    printk(
        "Unexpected address 0x%x, RamSize = 0x%x\n",
        (unsigned int)ea,
  ...
  if ( ppteg_search_status == -1 ) {
    ...
    if ( spteg_search_status == -1 ){
      ...
      if (
          ((key && (pp == 1 || pp == 0 || pp == 3)) || (~(key) && (pp == 3)))
          && ( cause & 0x02000000 )
      ) {
        /* Write access denied */
        #ifdef DEBUG
          printk("Error: Write access denied address 0x%x\n", ea);
        #endif
        /* FIXME: Use fatal error? */
        rtems_task_delete(RTEMS_SELF);
      } else if ( (key && (pp == 0)) && (~cause & 0x02000000) ) {
        /* Read access denied */
        #ifdef DEBUG
          printk("Error: Read access denied address 0x%x\n", ea);
        #endif
        /* FIXME: Use fatal error? */
        rtems_task_delete(RTEMS_SELF);
      } else {
